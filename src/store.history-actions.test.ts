...
import { FLOW_COVER_INVALIDATED_EVENT, useFlowStore } from './store';
...
describe('persisted history actions',()=>{
  ...
  it('activates without structural undo and rebuilds typed scalar output',async()=>{
    ...
    expect(dispatchEvent).toHaveBeenCalledWith(expect.objectContaining({type:FLOW_COVER_INVALIDATED_EVENT,init:expect.objectContaining({detail:expect.objectContaining({documentId:expect.any(String)})})}));

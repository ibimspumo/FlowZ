...
export function layerKeyboardGeometryPatch(key:string,resize:boolean,step=1):Partial<ArtboardLayer["geometry"]>|undefined {
  const amount=Math.max(1,step);
  if(resize) {
    if(key==="ArrowLeft")return{width:-amount};
    if(key==="ArrowRight")return{width:amount};
    if(key==="ArrowUp")return{height:-amount};
    if(key==="ArrowDown")return{height:amount};
    return;
  }
  if(key==="ArrowLeft")return{x:-amount};
  if(key==="ArrowRight")return{x:amount};
  if(key==="ArrowUp")return{y:-amount};
  if(key==="ArrowDown")return{y:amount};
}
    ...
    <div className={`awb-world ${props.viewportTransition ? "is-agent-fitting" : ""}`} style={{ transform: `translate(${props.pan.x}px, ${props.pan.y}px) scale(${props.zoom})` }}>
      {boards.map((board) => {
        ...
        const renderLayer = (layerId:string, ancestorLocked=false):ReactNode => {
          ...
          const visual=<button key={`${layer.id}-visual`} type="button" className={`awb-layer awb-layer-${layer.type} ${isSelected?"is-selected":""}`} style={{...common,...styleCss(layer),...(layer.type==="shape"||layer.type==="container"?{background:paintCss(layer.fill),borderRadius:layer.type==="shape"&&layer.shape==="ellipse"?"50%":layer.style?.borderRadius}:{}),...(layer.type==="text"?{color:layer.color,fontSize:layer.fontSize,textAlign:layer.align,fontFamily:customFontReady?artboardFontFamily(layer.fontHash!):layer.fontFamily,fontWeight:layer.fontWeight,fontStyle:layer.fontStyle,fontVariationSettings:layer.fontAxes?Object.entries(layer.fontAxes).map(([tag,value])=>`"${tag}" ${value}`).join(", "):undefined}:{}),...imageStyle,pointerEvents:"auto"}} onPointerDown={(event)=>{if(!locked)beginLayer(event,board.id,layer,"move");else{event.stopPropagation();props.onSelectLayer({boardId:board.id,layerId:layer.id});}}} onClick={(event)=>{event.stopPropagation();props.onSelectLayer({boardId:board.id,layerId:layer.id});}} onDoubleClick={(event)=>{event.stopPropagation();props.onSelectLayer({boardId:board.id,layerId:layer.id});}} onKeyDown={(event)=>{if(locked)return;const delta=layerKeyboardGeometryPatch(event.key,event.shiftKey,event.altKey?10:1);if(!delta)return;event.preventDefault();event.stopPropagation();const patch=event.shiftKey?{width:layer.geometry.width+(delta.width??0),height:layer.geometry.height+(delta.height??0)}:{x:layer.geometry.x+(delta.x??0),y:layer.geometry.y+(delta.y??0)};const geometry=clampLayerGeometry(layer,patch,board.document.format);props.onSelectLayer({boardId:board.id,layerId:layer.id});props.onCommit([{type:"update-layer",boardId:board.id,layerId:layer.id,patch:{geometry,version:layer.version+1} as Partial<ArtboardLayer>}]);}} aria-label={locked?t('artboard.layerLocked',{name:layer.name}):layer.name} aria-pressed={isSelected} aria-keyshortcuts={locked?undefined:"ArrowUp ArrowDown ArrowLeft ArrowRight Shift+ArrowUp Shift+ArrowDown Shift+ArrowLeft Shift+ArrowRight"} data-board-id={board.id} data-layer-id={layer.id}>
            {layer.type==="text"?layer.text:null}{locked?<Lock className="awb-layer-lock" size={14/props.zoom}/>:null}{isSelected&&!locked?<span className="awb-resize" aria-hidden="true" onPointerDown={(event)=>beginLayer(event,board.id,layer,"resize")}><MoveDiagonal2 size={16/props.zoom}/></span>:null}
          </button>;
